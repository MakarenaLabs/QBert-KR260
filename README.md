...
This project aims to port the QBert cabinet emulator to the Xilinx KR260 FPGA. The QBert emulator allows you to play QBert cabinet games on FPGA hardware.
...
- `tools`: contains scripts for converting a QBert games file to COE format for loading the ROM into the EPROM(s) IPs.

## Loading a New Cartridge statically

To load an QBert cartridge statically, generate the COE file and load it via Vivado interface. Follow these steps:
...
* Original FPGA conversion by Pierco
